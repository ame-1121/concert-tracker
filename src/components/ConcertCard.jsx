...
const statusConfig = {
  ...
  '免费': { cls: 'status-free', label: '免费' },
  '已结束': { cls: 'status-ended', label: '已结束' },
};

function formatDate(dateStr) {
  const date = new Date(dateStr);
  // 修复时区问题：new Date('2026-08-23') 可能解析为 UTC 时间
  ...
  let relative = '';
  if (diffDays < 0) relative = '已结束';
  else if (diffDays === 0) relative = '今天';
  ...
  else if (diffDays > 30 && diffDays <= 90) relative = `${Math.ceil(diffDays / 30)}月后`;
  else relative = '';
  ...
  return (
    <div className={`concert-card ${isInPlaylist ? 'card-matched' : ''}`}>
      ...
      <div className="card-type" style={{ background: typeInfo.bg }}>
        ...
        {c.verified && <span className="verified-badge" title="信息已验证">✓</span>}
      </div>
      ...
      <div className="card-body">
        ...
        <div className="card-meta">
          <div className="meta-item">
            ...
            <span className="meta-value">
              ...
              {dateInfo.relative && (
                <span className={`meta-relative ${dateInfo.relative === '已结束' ? 'ended' : ''}`}>
                  {' · '}{dateInfo.relative}
                </span>
              )}
            </span>
          ...
          {c.price && (
            <div className="meta-item">
              <span className="meta-icon">💰</span>
              <span className="meta-value price-text">{c.price}</span>
            </div>
          )}
        </div>
        ...
        <div className="card-footer">
          ...
          <span className="source-tag" title={c.source}>📋 {c.verified ? '✅ ' : ''}{c.source}</span>
        </div>
      </div>
      ...
      {c.status === '免费' && (
        <div className="card-action card-action-free">
          🆓 免费入场
        </div>
      )}
      {c.status === '已结束' && (
        <div className="card-action card-action-ended">
          ⏰ 已结束
        </div>
      )}
      {c.ticketUrl && c.status === '即将开票' && (
        <a href={c.ticketUrl} target="_blank" rel="noopener noreferrer" className="card-action card-action-watch">
          🔍 关注开票
        </a>
      )}
    </div>
